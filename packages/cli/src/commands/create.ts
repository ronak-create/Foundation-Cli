...
export async function runCreateCommand(
  ...
  const flowOpts: { preset?: string; ciMode?: boolean } = {
    ciMode: !!opts.preset,
  };

  if (opts.preset !== undefined) {
    flowOpts.preset = opts.preset;
  }

  const selection = await runPromptFlow(flowOpts);
...
function buildReadme(projectName: string, s: UserRawSelections): string {
  ...
  return `# ${projectName}\n\n[![Built with Foundation](https://img.shields.io/badge/built%20with-Foundation%20CLI-0ea5e9?style=for-the-badge&logo=typescript)](https://github.com/your-org/foundation-cli)\n\n## Stack\n\n${stack || "- Custom configuration"}\n\n## Getting Started\n\n\`\`\`bash\ncd ${projectName}\nnpm install\nnpm run dev\n\`\`\`\n\n## Environment Variables\n\nCopy \`.env.example\` to \`.env\` and fill in your values:\n\n\`\`\`bash\ncp .env.example .env\n\`\`\`\n`;
}
